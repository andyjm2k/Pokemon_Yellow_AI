...
class GbaGame(Env):
    def __init__(self, max_episodes=400000):
        ...
        self.observation_space = Box(low=0, high=255, shape=(72, 80, 1), dtype=np.uint8)
        ...
        if self.is_battling_fl:
            if v_3 == 3:
                ...
                if self.battle_stuck_counter > 1000:
                    stuck = True
                    self.ash_stuck_counter += 1
                    # print("battle is stuck count=",self.battle_stuck_counter)
                    return stuck
                else:
                    return stuck
